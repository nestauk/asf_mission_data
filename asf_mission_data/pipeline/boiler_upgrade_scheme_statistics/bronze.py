"""Hamilton nodes for bronze-layer of the Boiler Upgrade Scheme Statistics pipeline"""
...
from asf_mission_data.pipeline.boiler_upgrade_scheme_statistics.validators import ExcelFileExtensionValidator, WithinLastCalendarMonthValidator

logger = logging.getLogger(__name__)


def latest_release_api_response(collection_url: str) -> dict:
    """Fetch the latest BUS stats release metadata from the GOV.UK Content API."""
    return utils.fetch_govuk_latest_release_api_response(collection_url)


def latest_release_page_url(latest_release_api_response: dict) -> str:
    """Extract the URL of the latest BUS stats release page."""
    return utils.extract_govuk_latest_release_page_url(latest_release_api_response)


def latest_file_url(
    latest_release_api_response: dict,
) -> str:
    """Extract the latest BUS stats Excel file URL from the GOV.UK Content API response."""
    return utils.extract_govuk_latest_release_file_url(latest_release_api_response)


def latest_file_content(latest_file_url: str) -> bytes:
    """Fetch the raw content of the latest BUS stats file from given URL."""
...
def latest_filename(latest_file_url: str) -> str:
    ...
    logger.info("Selected workbook: %s", filename)
    return filename


@check_output_custom(WithinLastCalendarMonthValidator())
def latest_publication_date(latest_release_api_response: dict) -> str:
    ...
    logger.info("Detected publication date: %s", publication_date)
...
def bronze_boiler_upgrade_scheme_statistics_file(
    dataset_prefix: str,
    ...
    logger.info(
        "Writing Boiler Upgrade Scheme Statistics bronze dataset: filename=%s publication_date=%s",
        latest_filename,
